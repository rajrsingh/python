...
RETHINKDB_URL = os.environ['RETHINKDB_URL']
PATH_TO_CERT = os.environ['PATH_TO_CERT']
parsed = urlparse(RETHINKDB_URL)
conn = r.connect(
    host=parsed.hostname,
    port=parsed.port,
    user=parsed.username,
    password=parsed.password,
    ssl={'ca_certs': PATH_TO_CERT})
...
def handle_words(name=None):
    ...
    conn.use("grand_tour")
    r.table("python").insert(new_word).run(conn)
    return "ECHO: PUT\n"
...
def display_find(name=None):
    conn.use("grand_tour")
    cursor_obj = r.table('python').pluck("word", "definition").run(conn)
